...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    @Query(value = "select n from Notification n join n.users u on u.id = :userId")
    ...
    @Query(value = "select n from Notification n join n.users u on u.id = :userId order by n.id desc ")
    Page<Notification> recentNotifications(@Param("userId") Long userId, PageRequest pageable);

    @Query(value = "select count(n) from Notification  n join n.users u on u.id = :userId")
